...
class Analyze(QWidget):
    ...
    def set_qa_data(self, questions, correct_answers, user_answers):
    ...
    def update_score(self, score, wrong_indices=None):
        ...
        # 데이터가 있을 때만 분석
        if self.questions and self.correct_answers and self.user_answers:
            self.analyze_with_gemini()
        else:
            self.lack_text.setText("분석할 데이터가 없습니다.")
